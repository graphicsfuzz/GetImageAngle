...
void main()
{
    ...
    vec3 camUp  = normalize(vec3(cos(time/4.0), sin(time/4.0), 0.15));
    ...
    vec3  fog2 = 0.01 * vec3(1.1, 1, 1.0) * total_d * 1.4;
    glow *= min(1.0, 4.0-(4.0 / float(MAX_MARCH-1)) * float(march));
    float scanline = mod(gl_FragCoord.y, 4.0) < 2.0 ? 0.7 : 1.0;
    gl_FragColor = vec4(vec3(0.01+glow*0.1, 0.15+glow*0.5, 0.15+glow*0.5)*fog + fog2, 1.0) ;//* scanline;
}